...
def main(cfg):
    ...
    if "llava" in cfg.model_id.lower():
        ...
        if hasattr(model.config, "vision_config"):
            model.vision_tower.patch_size = model.config.vision_config.patch_size
        else:
            # Fallback if config is nested differently
            model.vision_tower.patch_size = 14        
        ...
        if cfg.loss_type == "KL":
            oracle_model = LlavaForConditionalGeneration.from_pretrained(cfg.model_id, attn_implementation="sdpa", torch_dtype=torch.float16, device_map="auto")
        if cfg.LoRA.r != 0:
